...
namespace Arbolito\UI\components\xls\producto;
	...
	protected function parseXTemplate(XTemplate $xtpl){
		...
		foreach ($tipos as $tipo) {
			...
			foreach ($productos as $producto) {
				...
				$xtpl->assign( "precio", ArbolitoUIUtils::formatMontoToView( $producto->getPrecioEfectivo() ) );
			...
			foreach ($arrTipos as $arrTipo) {
				...
				foreach ($packs as $pack) {
					if ($arrTipo->getOid()==$pack->getProducto()->getTipoProducto()->getOid()) {
						$html.='<tr><td>'.$pack->getProducto()->getMarcaProducto().' '.$pack->getProducto()->getNombre().' '.$pack->getNombre().'</td><td style="text-align: right;">'.$pack->getCantidad().'</td><td style="text-align: right;">'.ArbolitoUIUtils::formatMontoToView($pack->getPrecioEfectivo()).'</td></tr>';
					}
			...
			if ($mostrarCombo) {
				$html.='
								...
								foreach ($arrayCombos as $combo) {
									$productos='(';
									foreach ($combo->getProductos() as $value) {
										$productos.= $value->getCantidad(). ' '.$value->getProducto()->getNombre().' '.$value->getProducto()->getMarcaProducto().' '.ArbolitoUIUtils::formatMontoToView($value->getPrecioUnitario()).'-';
									}
									$productos.=')';
										$html.='<tr><td>'.$combo->getNombre().' '.$productos.'</td><td style="text-align: right;">'.ArbolitoUIUtils::formatMontoToView( $combo->getPrecio() ).'</td></tr>';
