...
namespace Arbolito\UI\components\boxes\venta;
...
class VentaBox extends RastyComponent{
	...
	protected function parseXTemplate(XTemplate $xtpl){
		...
		$xtpl->assign( "monto", ArbolitoUIUtils::formatMontoToView( $this->getVenta()->getMonto() ) );
		$xtpl->assign( "montoPagado", ArbolitoUIUtils::formatMontoToView( $this->getVenta()->getMontoPagado() ) );

		$xtpl->assign( "montoDebe", ArbolitoUIUtils::formatMontoToView( $this->getVenta()->getMontoDebe() ) );
		$xtpl->assign( "montoTotal", $this->getVenta()->getMontoDebe() );


		$xtpl->assign( "observaciones", $this->getVenta()->getObservaciones() );
		$xtpl->assign( "fecha", ArbolitoUIUtils::formatDateTimeToView($this->getVenta()->getFecha()) );
		...
		foreach ($venta->getDevoluciones() as $devolucion) {
			...
			$xtpl->assign( "precio", ArbolitoUIUtils::formatMontoToView( $devolucion->getPrecioUnitario() ) );
			$xtpl->assign( "subtotal", ArbolitoUIUtils::formatMontoToView( $devolucion->getSubtotal() ) );
			$xtpl->parse( "main.devolucion" );*/

			$htmlItem .='<tr><td>'.$devolucion->getProducto().'</td><td class="align-r">'.$devolucion->getCantidad().'</td><td class="align-r">'.ArbolitoUIUtils::formatMontoToView( $devolucion->getPrecioUnitario() ).'</td><td class="align-r">'.ArbolitoUIUtils::formatMontoToView( $devolucion->getSubtotal() ).'</td></tr>';
		...
		if ($cantidadTotal) {
			$html.='<div class="box_details">
					<table>
						...
						<tfoot>
							<tr><td>'.$this->localize( "venta.detalle.subtotal" ).'</td><td class="align-r">'.$cantidadTotal.'</td><td></td><td class="align-r">'.ArbolitoUIUtils::formatMontoToView( $venta->getMontoDevolucion() ).'</td></tr>
						</tfoot>
					</table>
				</div>';
		}
