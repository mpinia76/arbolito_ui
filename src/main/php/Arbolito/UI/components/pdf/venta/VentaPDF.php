...
class VentaPDF extends RastyComponent{
	...
	protected function parseXTemplate(XTemplate $xtpl){
		...
		if( !empty($venta )){
			...
			$xtpl->assign( "fecha", ArbolitoUIUtils::formatDateToView($venta->getFecha()) );
            ...
            $xtpl->assign( "nroSocio", $venta->getCliente()->getNroSocio());
			...
			foreach ($venta->getDetalles() as $detalle) {
				if (!empty($detalle->getCombo())) {
					...
					if(!in_array($detalle->getCombo()->getOid(), $combos)){
						...
						$xtpl->assign( "precio", ArbolitoUIUtils::formatMontoToView( $detalle->getCombo()->getPrecio()*$cantidad ) );
						$xtpl->assign( "subtotal", ArbolitoUIUtils::formatMontoToView( $detalle->getCombo()->getPrecio()*$cantidad  ) );
			...
			foreach ($venta->getDevoluciones() as $devolucion) {
				...
				$htmlItem .='<tr><td>'.$devolucion->getProducto().'</td><td style="text-align: right;">'.$devolucion->getCantidad().'</td><td style="text-align: right;">'.ArbolitoUIUtils::formatMontoToView( $devolucion->getPrecioUnitario() ).'</td><td style="text-align: right;">'.ArbolitoUIUtils::formatMontoToView( $devolucion->getSubtotal() ).'</td></tr>';
	...
	<table cellspacing="0" cellpadding="0" border="1">
		...
		<tr><td colspan="4"></td></tr>
		<tr><td colspan="4"></td></tr>
			<tr><td style="text-align: right; font-weight: bold;" colspan="3">'.$this->localize( "venta.detalle.subtotal" ).'</td><td style="text-align: right;font-weight: bold;">'.ArbolitoUIUtils::formatMontoToView( $venta->getMontoDevolucion() ).'</td></tr>
			...
			if ($venta->getMontoActualizado()) {
				$xtpl->assign( 'htmlMontoActualizado','<tr><td style="text-align: right; font-weight: bold;" colspan="3" width="616px">'.$this->localize('venta.montoActualizado').'</td><td style="text-align: right;font-weight: bold;" width="100px">'.ArbolitoUIUtils::formatMontoToView($venta->getMontoActualizado()),'</td></tr>');
			}
			$xtpl->assign( "totales", ArbolitoUIUtils::formatMontoToView( $venta->getMonto() ) );
			$xtpl->assign( "pagado", ArbolitoUIUtils::formatMontoToView( $venta->getMontoPagado() ) );
			$xtpl->assign( "adeudado", ArbolitoUIUtils::formatMontoToView( $venta->getMontoDebe() ) );
