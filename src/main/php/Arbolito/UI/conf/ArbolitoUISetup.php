...
/**
 * configuración Arbolito ui
 *
 * @author Marcos
 * @since 28/02/2018
 */
class ArbolitoUISetup {

	/**
	 * inicializamos la aplicación de cuentas ui
	 */
	public static function initialize( $appName = "arbolito_ui"){
...
//		$_SESSION['LAST_ACTIVITY'] = time();
	...
	private static function initializeCore(){
		...
		ArbolitoConfig::getInstance()->setDbName("cose_arbolito");

		ArbolitoConfig::getInstance()->initialize();
		ArbolitoConfig::getInstance()->initLogger(dirname(__DIR__) . "/conf/log4php.xml");
	...
	private static function initializeUI( $appName ){
		...
		$rastyLoader->loadXml(
				...
				RastyConfig::getInstance()->getAppPath() . "src/main/php/Arbolito/UI/",
		...
		//SorteosUIConfig::getInstance()->initialize("ArbolitoMetroLayout");
