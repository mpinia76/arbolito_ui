<?php
namespace Arbolito\UI\service\finder;
 ...
 * @since 10/06/2021
 */
class TipoClienteFinder implements  IAutocompleteFinder {
	...
	public function findEntitiesByText( $text, $parent=null ){
		...
		$uiCriteria = new UITipoClienteCriteria();
		$uiCriteria->setNombre( $text );
		$uiCriteria->setRowPerPage( 10 );
		return UIServiceFactory::getUITipoClienteService()->getList($uiCriteria);
	}
	...
	function findEntityByCode( $code, $parent=null ){
		...
		return UIServiceFactory::getUITipoClienteService()->get( $code );
	}
