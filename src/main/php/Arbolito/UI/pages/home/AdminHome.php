<?php
namespace Arbolito\UI\pages\home;

use Arbolito\UI\components\filter\model\UIBancoCriteria;
use Arbolito\UI\pages\ArbolitoPage;

use Arbolito\UI\components\filter\model\UIEmpleadoCriteria;
...
class AdminHome extends ArbolitoPage{
	...
	public function __construct(){
		...
		/*if( ArbolitoUIUtils::isCajaSelected() ){
			$caja = UIServiceFactory::getUICajaService()->get(ArbolitoUIUtils::getCaja()->getOid());
			$this->setCaja( $caja );
		}*/
	...
	public function parseStatsVentas(XTemplate $xtpl){
		...
		$xtpl->assign("monto_ventas", ArbolitoUIUtils::formatMontoToView($totalesDia["monto"]) );
		...
		$xtpl->assign("monto_ventas_mes", ArbolitoUIUtils::formatMontoToView($totalesMes["monto"]) );
	...
	public function parseDebitosCreditos( XTemplate $xtpl){
		...
		foreach ($debitosCreditos as $informeDebitoCredito) {
			...
			$xtpl->assign("titulo", ArbolitoUIUtils::formatDateToView( $informeDebitoCredito->getFechaVencimiento()) );

			if($informeDebitoCredito->getDebito()){
				$xtpl->assign("subtitulo", ArbolitoUIUtils::formatMontoToView($informeDebitoCredito->getDebito()) );
				$xtpl->assign("descripcion", "Débito" );
			}else{
				$xtpl->assign("subtitulo", ArbolitoUIUtils::formatMontoToView($informeDebitoCredito->getCredito()) );
	...
	public function parseSaldos(XTemplate $xtpl){
        ...
        $xtpl->assign("saldo_bancos", ArbolitoUIUtils::formatMontoToView( UIServiceFactory::getUIBancoService()->getSaldoBancos($uiCriteria) ) );
        ...
        foreach ($bancos as $banco){
            ...
            $xtpl->assign("saldo_cuenta", ArbolitoUIUtils::formatMontoToView( UIServiceFactory::getUIBancoService()->getSaldoBanco($uiCriteria) ) );
	...
	public function parseGastos( XTemplate $xtpl){
		...
		foreach ($gastos as $gasto) {
			$xtpl->assign("titulo", ArbolitoUIUtils::formatDateToView( $gasto->getFechaVencimiento()) );
			$xtpl->assign("subtitulo", ArbolitoUIUtils::formatMontoToView($gasto->getMonto()) );
